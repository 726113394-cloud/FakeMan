...
- **完整背包系统**：完整的玩家背包结构（36格主背包 + 4格护甲栏 + 1格副手栏），右键假人即可打开
...
### 4. 打开假人背包
- **右键点击假人**即可打开完整的玩家背包界面
- 包含：36格主背包、4格护甲栏、1格副手栏
- 假人背包内容会自动保存
- 假人会自动使用背包中的工具（镐子、斧头、武器等）
...
- **完整背包系统**：完整的玩家背包结构（36格主背包 + 4格护甲栏 + 1格副手栏）
- **背包保存修复**：修复背包物品丢失问题，关闭背包时自动保存
- **智能工具切换**：假人自动从背包中寻找并使用工具（镐子、斧头、武器等）
- **安全增强**：防止假人掉入虚空，危险位置自动传送
- **工具检查**：假人自动检查背包中是否有对应工具/食物
- **玩家列表显示**：Citizens假人显示为在线玩家
- **配置版本**：更新至ScriptIrc-config-version: 4
...
**插件作者：来自太空的小头脑**  
**B站主页：https://space.bilibili.com/3493116665400113**  
**感谢使用 FakeMan 假人插件！**
